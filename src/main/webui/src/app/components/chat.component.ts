    ...
    <div class="chat-container" [class.minimized]="isMinimized()">
      @if (isMinimized()) {
        ...
        <div class="chat-input">
          ...
          <!-- Send button -->
          <button class="send-button" (click)="sendMessage()" [disabled]="!currentMessage() && selectedFiles().length === 0">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
...
export class ChatComponent implements AfterViewInit {
  ...
  sendMessage(): void {
    ...
    const userMessage: ChatMessage = {
      id: Date.now(),
      text: this.currentMessage() || (this.selectedFiles().length > 0 ? 'Sent attachments' : ''),
      sender: 'user',
      timestamp: new Date(),
  ...
  private addBotResponse(): void {
    // In a real app, you would get the response from a service
    const randomIndex = Math.floor(Math.random() * this.botResponses.length);
    const botMessage: ChatMessage = {
      id: Date.now(),
      text: this.botResponses[randomIndex],
      sender: 'bot',
      timestamp: new Date()
    };
